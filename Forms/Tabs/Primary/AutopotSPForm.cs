        ...
        private void UpdatePanelLayoutAndData()
        {
            ...
            for (int i = 0; i < spPanels.Count; i++)
            {
                ...
                foreach (Control control in panel.Controls)
                {
                    ...
                    if (control is CheckBox chk) { chk.CheckedChanged -= OnSPEnabledChanged; chk.Checked = slot.Enabled; chk.CheckedChanged += OnSPEnabledChanged; }
                    if (control is TextBox txt) { txt.TextChanged -= OnSPKeyChanged; txt.Text = slot.Key.ToString(); txt.TextChanged += OnSPKeyChanged; }
                    if (control is NumericUpDown num) { num.ValueChanged -= OnSPPercentChanged; num.Value = slot.SPPercent; num.ValueChanged += OnSPPercentChanged; }
                }
            }
        }
